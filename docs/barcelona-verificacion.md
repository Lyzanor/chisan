...
- Filas: **2.881** · `verificado` **520** · `parcial` **207** · `pendiente` **2.154**
- Snapshot inicial era 2.973 · 35 · 16 · 2.922 (se han **purgado 92 filas** y verificado/parcial el resto).
- Modo: **verificación profunda**, **lote a lote bajo demanda** (~25 filas/lote). ~90 lotes estimados.
- **Cerrados:** Lotes 1-18.
  **Lote 18 = Sant Cugat del Vallès** COMPLETO (26 pend en una pasada: 20 verif + 5 parcial + 1 purga).
  **Comerç de ciutat adinerada** (forns/pastisseries, xarcuteries, xocolata, cervesa) — gairebé tot real
  amb web pròpia i ecommerce (molt diferent del clúster de registre de Sant Boi). **VO sí/ecommerce:** La
  Fem (cervesa, 2 sedes), Obrador Griful (65a), Clorawfila (bean-to-bar premiat), Carns Corella (×2 sedes,
  "Comanda Online"), Carlota's, Famatina, La Rústica, OMG sense gluten, Yann Duytsche, Rabassaires (celler
  urbà del Vallès, +web/tel/correu). **VO sí/marketplace:** Bacallaneria H. Antón (37a, elaboració pròpia,
  ven a `botiga.mercattorreblanca.cat`; FB/IG del mercat blanquejats). **VO no:** Bonaparte, Serrajòrdia
  (1964), Pastisseria Sàbat (×2 sedes, 1951), Cap al Forn ("50 millors fleques d'Espanya", web=directori
  santcugatcomerc **blanquejat**, IG real), Family Bakery, Melvida (mel crua Collserola, **recat Despensa→
  Miel**, +web melvida.cat). **Recats:** Tocat de l'Ala era "Fruta y verdura" → és **rostisseria** (recat
  **Platos preparados**, +tel/FB). **Parcial:** Agrícola Domènech (Can Domènech, catàleg payesia Collserola),
  Masia Can Bell, Gasull & Claramunt (llegums, parada Mira-sol), Xarcuteria Cristóbal (parada Mira-sol), Téo
  (xocolater, només IG). **1 purga:** Barbero Iserna Eric = dup de Melvida (mateix correu, coords +25km).
- **Siguiente:** Lote 19 = **Moià** (38 pendientes → ~2 sub-lotes).
- Último push: **lotes 1-18 en `main`** (17 y 18 pusheados en este turno).
