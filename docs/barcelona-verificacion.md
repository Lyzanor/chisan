...
- Filas: **2.882** · `verificado` **500** · `parcial` **202** · `pendiente` **2.180**
- Snapshot inicial era 2.973 · 35 · 16 · 2.922 (se han **purgado 91 filas** y verificado/parcial el resto).
- Modo: **verificación profunda**, **lote a lote bajo demanda** (~25 filas/lote). ~90 lotes estimados.
- **Cerrados:** Lotes 1-17.
  **Lote 17 = Sant Boi de Llobregat** COMPLETO (34 pend en una pasada: 10 verif + 15 parcial + 9 purgas).
  **Clúster del Parc Agrari del Baix Llobregat** (todo "Fruta y verdura"; muchos noms de registre + webs
  auto-omplertes a `parcagrari.cat/pagesia/productor` genèric). Fuentes clave: **DAR** `xmyy-7xqi`, el
  **directorio del Parc Agrari** (`parcagrari.cat/pagesia/productors/<slug>`, registre real → parcial) i
  **fitxes municipals** `santboi.cat/<nom>` (cal-pereto, cal-notari). **4 dups registre↔marca** detectats
  pel DAR i purgats: Bou Samsó=**Cal Coracero**, Chantry=**Cal Notari**, Domínguez Farrés=**El Nano
  Farinetes**, Chaabi Carrilero=**Cal Pastera Eco**. **Cross-links aliens netejats:** Ros Prat→`daciaros.com`
  =**concessionari Dacia** (blanquejat), Joan Solé→`santboi.cat/tramits`+tel ajuntament (purgat), Chantry
  →`consum.es` (era dup). **Botigues online reals (VO sí):** Coop. Agrària Santboiana (web corregida
  `negocio.site`→`agrariasantboiana.cat`), Cal Peretó (cistelles eco CCPAE), Bierboi (cervesa, ecommerce).
  **Brot Nou** tenia `sí/ecommerce` **erroni** (només parada Mercabarna)→VO no. Mas Llopis (=Capdevila
  Gibert) +web/FB/marca DAR. **5 purgues sense rastre:** Cal Penyasco (toponim=viver Garden l'Aralia),
  Blecamp SL, Hortofrutícola Obiols, Ricard Ros Puig (coords +40km), Joan Solé Balagué. Sense imatges òrfenes.
- **Siguiente:** Lote 18 = **Sant Cugat del Vallès** (26 pendientes → ~2 sub-lotes).
- Último push: **lotes 1-16 en `main`** (15 y 16 ya pusheados). **Pendiente de push: lote 17** (este commit).
