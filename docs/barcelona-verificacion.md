...
- Filas: **2.899** · `verificado` **401** · `parcial` **171** · `pendiente` **2.327**
- Snapshot inicial era 2.973 · 35 · 16 · 2.922 (se han **purgado 74 filas** y verificado/parcial el resto).
- Modo: **verificación profunda**, **lote a lote bajo demanda** (~25 filas/lote). ~93 lotes estimados.
- **Cerrados:** Lotes 1-13. Lote 11 Vic (46), Lote 12 Vilanova i la Geltrú (40),
  **Lote 13 = Vilafranca del Penedès** COMPLETO (38 pend en una pasada: 28 verif + 10 parcial + **0 purgas**).
  Capital del Penedès: muchos cellers/cavas reales (Mascaró, Pinord, Mastinell, Mulet, Trias Batlle, Tarrida
  i Sibil, Soler Gatell/Mas Gatell, El Mas Pujó). Patrón estrella: **webs cruzadas auto-rellenadas a entidades
  ajenas** (Cal Vives→`calvives.com` es la de **Moià**; Gelats Xixona→`gelatsitorronsxixona.com` es la de
  **Reus**, su web real es `botigatorrons.com`; Jose Gol→`santjosep.cat`=**Escola Sant Josep**; Joan Ferran
  Roses→`smp.cat`=**clínica Serveis Mèdics Penedès**; Pascual Leon→constructora). DAR Alt Penedès confirmó
  Gol Llenas (POMES), Domènech Rovira (ROSA DOMENECH), Mascaró Figuerola (PATATA). Valliser estaba mal
  fichado: es **membrillo/codonyat artesano en Vilobí del Penedès** (no fruta/verdura de Vilafranca).
- **Siguiente:** Lote 14 = **Sant Sadurní d'Anoia** (47 pendientes → ~2 sub-lotes).
- Último push: lote 12 en `main`. **Pendiente de push: lote 13** (commit local hecho).
