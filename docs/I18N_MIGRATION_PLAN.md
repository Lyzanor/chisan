...
changing producer or account identity. The framework migration and first
controlled rollout were completed on 2026-08-24. Keep this plan until the
...
Implementation checkpoint on 2026-08-24:
...
- Phases 0–11 are complete, including explicit source locales on all 695 area
  ...
  invariants.
- Phase 12 is complete for the first rollout: Catalunya publishes Catalan,
  Spanish and English; Germany publishes German and English; Japan publishes
  Japanese and English.
- The rollout contains 10,746 materialized description translations across
  seven country-level sidecars. Its publication review covered 2,003 entries,
  including every one of the 1,726 digit-bearing quantitative translations and
  all 28 detected written-number or ordinal translations. Rejected entries were
  regenerated by a different agent and accepted by the rejecting reviewer; the
  final gate revalidated all 10,746 rows against their canonical sources.
- Later Phase 12 country activations remain gated by their local-language and
  area policy review. Phase 13 is intentionally deferred until those targeted
...
registration. Belgium, India, Ireland and South Africa require explicit
area-level language review before activating new defaults; do not infer one
national language from the ISO country code.
...
- presentation locales remain a narrower registry (`en`, `es`, `ca`, `de`,
  `ja` at this stage), so accepting source-only prose never activates a route,
...
lib/i18n/messages/{en,es,ca,de,ja}.ts
...
The first provider selection must use a reviewed benchmark containing at least
50 representative descriptions for each of `ca`, `de` and `ja`, including:
...
4. Use language-region `hreflang` for territorial local variants (`ca-ES`,
   `es-ES`, `de-DE`, `ja-JP`) and generic `en` for English pages aimed at any
   English-speaking visitor. Do not mechanically copy the URL token into
   `hreflang`.
