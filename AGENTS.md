...
- `npx pnpm check:evidence`: non-blocking audit that validates every existing evidence record against its province CSV. Reports any mismatch as a warning (never fails the gate); the strict provinces in `data/evidence/coverage.json` are advisory and no longer require full coverage.
- `npx pnpm check:evidence:changed`: warning-only companion that diffs changed CSVs against HEAD and flags new producers, changed `verificacion`/`Venta online`, or removed rows whose evidence ledger line was not also updated. Non-blocking, like `check:evidence` itself; use it while iterating to spot decisions that still lack provenance.
...
- Evidence is an optional audit layer, not a gate. Prefer adding a structured record at decision time — when you add a producer, change `verificacion`, resolve `Venta online`, or purge/merge — because the sources are already open. Its absence never blocks: `check:evidence` is non-blocking and `coverage.json` strict coverage is advisory, so do not retroactively backfill an already-verified province just to satisfy it.
