...
import { withdrawProducerClaimAction } from "@/app/cuenta/actions";
...
export default async function ClaimsPage({ searchParams }: ClaimsPageProps) {
  ...
  const [claims, memberships, params] = await Promise.all([
    database
      .select()
      .from(producerClaims)
            ...
            {claims.map((claim, index) => {
              ...
              return (
                <li key={claim.id} className="account-record-list__stacked">
                  ...
                  {["draft", "pending", "needs_info"].includes(claim.status) ? (
