...
export async function updatePublicProfileQrAction(formData: FormData): Promise<void> {
  const account = await requireCurrentAccount("/cuenta/perfil");
  if (!account.termsAcceptedAt) redirect("/cuenta/bienvenida");
...
export async function updateProducerProfileQrAction(formData: FormData): Promise<void> {
