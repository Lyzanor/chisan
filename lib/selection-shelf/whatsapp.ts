...
const help = "Envía una foto de tu estantería. Chisan identificará los productos de tu selección pública, revisará los puntos y publicará la foto. ESTADO consulta el último envío; CANCELAR retira la foto pendiente; DESCONECTAR desvincula el número.";
...
export function createShelfWhatsAppHandler(deps: { catalog: ShelfCatalog; image: (image: NonNullable<InboundMessage["image"]>) => Promise<Buffer> }) {
  return async function handle(tx: AccountTransaction, message: InboundMessage): Promise<string | null> {
    ...
    // photo and inbox receipt commit together; inference happens after commit.
    ...
    if (command === "CANCELAR") {
      ...
      const pending = records.find((record) => ["queued", "processing", "review"].includes(record.status));
    ...
    try {
      ...
      return "Foto recibida. Chisan identificará los productos, revisará los puntos y publicará la foto. Si ya tienes una foto publicada, seguirá visible hasta que la nueva esté lista.";
    } catch (error) {
