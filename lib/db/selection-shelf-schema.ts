...
export const selectionShelves = pgTable("selection_shelves", {
  ...
  uniqueIndex("selection_shelves_pending_idx").on(table.userId).where(sql`${table.status} in ('queued','processing','review')`),
  index("selection_shelves_queue_idx").on(table.status, table.createdAt),
  check("selection_shelves_status_check", sql`${table.status} in ('queued','processing','review','published','rejected','superseded')`),
