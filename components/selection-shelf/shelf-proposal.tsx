...
import { ShelfEventRequest } from "./shelf-event-request";
import { ShelfCorrections } from "./shelf-corrections";
...
export function ShelfProposal({ proposal, items, profile, areas, allowed }: {
  ...
  const [checked, setChecked] = useState(proposal.eventRequest ? [...new Set(proposal.eventRequest.points.map((point) => point.producerKey))].filter((key) => proposal.producers.some((item) => item.key === key)) : proposal.producers.map((item) => item.key));
    ...
    try {
      const response = await fetch("/api/selection-shelf", { method: "PATCH", headers: { "Content-Type": "application/json", "X-Chisan-Shelf": "1" },
        body: JSON.stringify({ id: proposal.id, version: proposal.version, producerKeys: checked,
          ...(proposal.selectionId ? { selection: { title: data.get("title"), description: data.get("description") } } : { profile: { publicHandle: data.get("publicHandle"), baseLocation: data.get("baseLocation"), baseMunicipality: data.get("baseMunicipality") } }) }) });
  ...
  return <section aria-labelledby="shelf-proposal-title">
    ...
    {preparing ? <p role="status">Foto recibida. La IA está leyendo la imagen, buscando sus productores y colocando los puntos automáticamente. Esta página se actualizará; también puedes volver más tarde.</p> : null}
    {proposal.status === "review" || proposal.status === "received" ? <p role="status">{proposal.analysisError === "budget" ? "Foto recibida. El análisis está detenido porque se ha alcanzado el límite de llamadas de IA de Chisan. Chisan debe habilitar otro intento."
      ...
      : "La propuesta necesita una revisión de Chisan antes de estar lista."} Puedes completar las coincidencias manualmente. La imagen sigue siendo privada.</p> : null}
    {proposal.unmatched > 0 ? <p>{proposal.unmatched === 1 ? "1 nombre sin una coincidencia clara queda fuera de la propuesta." : `${proposal.unmatched} nombres sin una coincidencia clara quedan fuera de la propuesta.`}</p> : null}
    ...
    {allowed && ["ready", "review"].includes(proposal.status) ? <ShelfCorrections id={proposal.id} version={proposal.version} points={proposal.points} observations={proposal.observations} /> : null}
    {proposal.status === "ready" ? <form className="account-form" onSubmit={(event) => { event.preventDefault(); void publish(event.currentTarget); }}>
      <fieldset disabled={busy || !allowed} className={styles.editorPanel}>
        <legend>Comprueba tu selección</legend>
        ...
        <p>Los productores marcados formarán parte de esta selección y se mostrarán en su mapa y foto.</p>
        {proposal.selectionId ? <>
          <label>Nombre de la selección<input name="title" required maxLength={160} defaultValue={proposal.input.title} /></label>
          <label>Descripción pública (opcional)<textarea name="description" maxLength={600} placeholder="Qué reúne esta selección" /></label>
          <p>Publicaremos esta selección en su propia dirección. Tu perfil personal conserva su privacidad.</p>
        </> : <>
        ...
        <button className="chisan-button chisan-button--primary" disabled={busy || !checked.length}>{busy ? "Publicando…" : "Publicar mi selección"}</button>
      </fieldset>
    </form> : null}
    {allowed && ["ready", "published"].includes(proposal.status) ? <ShelfEventRequest id={proposal.id} version={proposal.version} title={proposal.input.title} request={proposal.eventRequest} producerKeys={checked} /> : null}
