...
CSV_PATH = 'data/csv/castilla-y-leon/burgos.csv'
OUTPUT_DIR = 'public/productores/burgos'
PUBLIC_PREFIX = '/productores/burgos'
...
def main():
    ...
    print(f"Processing {len(rows)} producers in Burgos...")
    ...
    for i, row in enumerate(rows):
        ...
        # Skip portal/generic URLs to avoid assigning generic portal logos to producers
        parsed_web = urlparse(web)
        domain = parsed_web.netloc.lower()
        if any(portal in domain for portal in ['facebook.com', 'instagram.com', 'twitter.com', 'youtube.com', 'gff.co.uk', 'burgosalimenta.com', 'burgosalimenta.es']):
            continue
            
        print(f"[{i+1}/{len(rows)}] Scouring web for {nombre} ({web})...")
        ...
        time.sleep(0.2) # Polite delay ( Burgos has 300+ entries, let's keep it relatively fast but respectful )
