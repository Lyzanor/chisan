...
CSV_PATH = 'data/csv/andalucia/cadiz.csv'
OUTPUT_DIR = 'public/productores/cadiz'
...
def main():
    ...
    print(f"Processing {len(rows)} producers in Cádiz...")
    ...
    for i, row in enumerate(rows):
        ...
        for score, src, url in scored_candidates:
            ...
            if download_and_process_image(url, output_path):
                row['imagen'] = f"/productores/cadiz/{output_filename}"
                success = True
                updated_count += 1
