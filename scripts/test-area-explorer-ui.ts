...
test("nearby search radius is 5 km by default and uses location-onboarding card without other distance filters", async () => {
  const { DEFAULT_SEARCH_RADIUS_KM } = await import("../lib/location/radius-search");
  const { getRadiusSearchMessages } = await import("../lib/i18n/radius-search");
  const { CatalogRadiusFilter } = await import("../components/catalog-radius-filter");

  assert.equal(DEFAULT_SEARCH_RADIUS_KM, 5);

  const esMessages = getRadiusSearchMessages("es");
  assert.equal(esMessages.title, "Cerca de mí");
  assert.match(esMessages.description, /5 km/);

  // When value is null, renders the location-onboarding style card with useLocation button and NO distance dropdown
  const htmlIdle = renderToStaticMarkup(createElement(CatalogRadiusFilter, {
    heading: "Productores",
    locale: "es",
    area: "España",
    value: null,
  ...
  assert.match(htmlIdle, /location-onboarding/);
  assert.match(htmlIdle, /Usar mi ubicación/);
  assert.doesNotMatch(htmlIdle, /<select/); // No distance dropdown for other distances!
  assert.doesNotMatch(htmlIdle, /10 km/);
  assert.doesNotMatch(htmlIdle, /25 km/);
  assert.doesNotMatch(htmlIdle, /50 km/);

  // When value is active (5 km), renders the active indicator with 5 km badge and clear button
  const htmlActive = renderToStaticMarkup(createElement(CatalogRadiusFilter, {
    heading: "Productores",
    locale: "es",
    area: "España",
    value: { latitude: 41.38, longitude: 2.17, radiusKm: 5 },
    count: 3,
    onChange: () => {},
  }));

  assert.match(htmlActive, /catalog-radius-heading/);
  assert.match(htmlActive, /5 km/);
