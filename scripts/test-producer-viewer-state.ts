...
test("private producer controls use only the current account and fail closed", async () => {
  ...
  try {
    ...
    assert.equal(owned.canClaimProducer, false);
    ...
    const otherProducer = await loadProducerViewerState("es", 2);
    assert.equal(otherProducer.membership, null, "another producer never inherits membership");
    assert.equal(otherProducer.canClaimProducer, false, "an owner cannot claim a second producer");
    ...
    assert.equal((await loadProducerViewerState("es", 2)).canClaimProducer, true);
