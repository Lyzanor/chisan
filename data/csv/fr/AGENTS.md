...
- `municipio` is the current official commune. A commune nouvelle replaces
  its absorbed name; retain the former commune or locality in `direccion`.
- Publish the producer's public brand when one exists. Do not invent a brand
...
- The inherited catalog came from the HVE certified-holding register. It proves
  only the certification facts it publishes; it does not establish a public
  producer identity, current activity, own offer or online sales.
- Legacy `web` values did not come from HVE and require an independent
  ownership match before they support any claim.
- The [Agence Bio directory](https://annuaire.agencebio.org/) and
  [open dataset](https://www.data.gouv.fr/datasets/professionnels-engages-en-bio)
  can locate certified operations, declared production and activity sites.
  Match each result to a public producer identity and productive unit before
  admission.
- The [Annuaire des Entreprises search API](https://recherche-entreprises.api.gouv.fr/)
  can discover active establishments by department and NAF code. Use it as a
  targeted discovery source, exclude holding, distribution and non-food
  activities, and require an explicit food signal where a code has mixed uses.
