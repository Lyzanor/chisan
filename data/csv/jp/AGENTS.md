...
## State
- 277 rows across 29 prefectures, 18 still empty files. 153 `verificado`, 124 `parcial`, no `pendiente`; 106 rows sell online.
...
- A roster that publishes domains is still a snapshot: in Nagano three of thirty-one were stale (a dead domain, an expired certificate, a redirect), so resolve each one before copying it into `web`. The same roster is the cheapest way to catch a closure — a brewery that a directory still lists and the association does not is worth a second look before adding it.
